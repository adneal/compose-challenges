...
private fun Flow(
    ...
    fun Placeable.mainAxisSize() = if (orientation == FlowOrientation.Horizontal) width else height
    fun Placeable.crossAxisSize() = if (orientation == FlowOrientation.Horizontal) height else width
    Layout(content) { measurables, outerConstraints ->
        ...
        val childConstraints = if (orientation == FlowOrientation.Horizontal) {
        ...
        fun canAddToCurrentSequence(placeable: Placeable): Boolean {
            return currentSequence.isEmpty() || currentMainAxisSize + mainAxisSpacing.roundToPx() +
        ...
        val layoutWidth = if (orientation == FlowOrientation.Horizontal) {
            mainAxisLayoutSize
        } else {
            crossAxisLayoutSize
        }
        val layoutHeight = if (orientation == FlowOrientation.Horizontal) {
            crossAxisLayoutSize
        } else {
            mainAxisLayoutSize
        }
...
internal data class OrientationIndependentConstraints(
    ...
    constructor(c: Constraints, orientation: FlowOrientation) : this(
        if (orientation === FlowOrientation.Horizontal) c.minWidth else c.minHeight,
        if (orientation === FlowOrientation.Horizontal) c.maxWidth else c.maxHeight,
        if (orientation === FlowOrientation.Horizontal) c.minHeight else c.minWidth,
        if (orientation === FlowOrientation.Horizontal) c.maxHeight else c.maxWidth
    )
    ...
    fun toBoxConstraints(orientation: FlowOrientation) = when {
        orientation === FlowOrientation.Horizontal -> Constraints(
            mainAxisMin, mainAxisMax, crossAxisMin, crossAxisMax
        )
        else -> Constraints(crossAxisMin, crossAxisMax, mainAxisMin, mainAxisMax)
    }

    // Given an orientation, resolves the max width constraint this instance represents.
    fun maxWidth(orientation: FlowOrientation) = when {
        orientation === FlowOrientation.Horizontal -> mainAxisMax
        else -> crossAxisMax
    }

    // Given an orientation, resolves the max height constraint this instance represents.
    fun maxHeight(orientation: FlowOrientation) = when {
        orientation === FlowOrientation.Horizontal -> crossAxisMax
        else -> mainAxisMax
    }
}
